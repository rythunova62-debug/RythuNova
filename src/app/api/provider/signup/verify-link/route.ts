...
export async function GET(request: Request) {
  ...
  const fail = () =>
    NextResponse.redirect(
      new URL(`/auth/provider/check-email?email=${encodeURIComponent(email)}&linkError=1`, url.origin)
    );
  ...
  return NextResponse.redirect(
    new URL(`/auth/provider/create-credentials?email=${encodeURIComponent(email)}`, url.origin)
  );
}
