...
function VerifyOtpForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/provider/signup/verify-otp", {
        method: "POST",
      ...
      router.push(`/auth/provider/create-credentials?email=${encodeURIComponent(email)}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleResend() {
    setError(null);
    setInfo(null);
    const res = await fetch("/api/provider/signup/resend-otp", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
